...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        #label
        self.lbl = tk.Label(self, text="Welcome to Mitigation Ronin (v47)", # {username}!", 
                            font=('Arial', 14), 
                            bg='#050505', 
                            fg='lightgreen')
        ...
        self.mit_lbl = tk.Label(self, text="Current Mitigations, Whitelists, and Reservations", # {username}!", 
                            font=('Arial', 11), 
                            bg='#050505', 
                            fg='lightgray')
        self.mit_lbl.grid(column=1, row=9, padx=10, pady=10, sticky='ns')
        self.unmit_lbl = tk.Label(self, text="Unmitigated Observables", # {username}!", 
                            font=('Arial', 11), 
                            bg='#050505', 
                            fg='lightgray')
        self.unmit_lbl.grid(column=1, row=12, padx=10, pady=10, sticky='ns')
        ######################################## IP box #############################################################
        ...
        def change_file():
            ...
            self.my_str.set(self.file)
        #self.upload_btn = tk.Button(self, text="Broken Upload Reference",command=change_file).grid(column=1, row=1, sticky="ns")
        ...
        self.update_btn.grid(column=1, row=14, sticky='ns', padx=500, pady=15)
        ...
        self.sub_sheet.enable_bindings(("single_select", #"single_select" or "toggle_select"
                                         ...
                                         "undo",
                                         "edit_cell"))
        self.sub_sheet.grid(column=1, row=13, padx=30, pady=5, sticky="nswe")
    ...
    def clicked_update(self):
        ...
        def onClick(): # update button command
            ...
            df1 = ip_df
            df1['First Binary'] = df1['First Binary'].apply(lambda x: int(ipaddress.ip_address(x.strip())))
            df1['Last Binary'] = df1['Last Binary'].apply(lambda x: int(ipaddress.ip_address(x.strip())))
            df1['Date Issued'] = df1['Date Issued'].apply(lambda x: (datetime.strptime(x, "%d-%b-%Y").date() - date(1899, 12, 30)).days)
            df2 = ipwhite_df
            df2['First Binary'] = df2['First Binary'].apply(lambda x: int(ipaddress.ip_address(x.strip())))
            df2['Last Binary'] = df2['Last Binary'].apply(lambda x: int(ipaddress.ip_address(x.strip())))
            df2['Date Issued'] = df2['Date Issued'].apply(lambda x: (datetime.strptime(x, "%d-%b-%Y").date() - date(1899, 12, 30)).days)
            df3 = self.df3
            df3['First Binary'] = df3['First Binary'].apply(lambda x: int(ipaddress.ip_address(x.strip())))
            df3['Last Binary'] = df3['Last Binary'].apply(lambda x: int(ipaddress.ip_address(x.strip())))
            df4 = self.df4
            df4['First Binary'] = df4['First Binary'].apply(lambda x: int(ipaddress.ip_address(x.strip())))
            df4['Last Binary'] = df4['Last Binary'].apply(lambda x: int(ipaddress.ip_address(x.strip())))
            df5 = dom_df
            ...
            save_file = filedialog.asksaveasfilename(initialdir=self.file,
                                                     ...
                                                     filetypes=[(f"References_{save_date}.xlsx", "*.xlsx")],
                                                     initialfile=f"References_{save_date}.xlsx",
    ...
    def clicked_ip(self):
        ...
        if ValueError:
            messagebox.showerror("Error", "IP does not appear to be an IPv4 or IPv6 address")
        ...
        self.sub_sheet.headers(newheaders=['Mitigated', 'Action', 'First Binary', "Last Binary", "CIDR", "Task Order", "Date Issued", "EvalReason","Threat Report", "Comments", "Notes", "Scope"])
    ...
    def clicked_dom(self):
        try:
            ...
            self.sub_sheet.headers(newheaders = ["Mitigated", "Action", "Domain", "Task Order", "Date Issued", "Threat Report", "Comments", "Notes"])
            self.sub_sheet.create_header_checkbox(c=0, text="Mitigate", checked=False, check_function=self.check_all)
            self.sub_sheet.create_checkbox(r="all",
                                           c=0,
                                           checked = False,
    ...
    def ip_search(self):
        ...
        mbox = f"{len(unmitigated)} unmitigated IP's found! I'm checking the whitelist now"#, {username} sempai!"
        #flags = uwuify.SMILEY | uwuify.YU
        messagebox.askokcancel(title='Results', message=mbox)
        ...
        df_ref['CIDR'] = df_ref['CIDR'].str.replace(' ', '')
        df_ref.drop_duplicates('CIDR')
        df_ref.insert(0, 'Mitigated', mitigations) # Adds mitigated IP's to df_ref for clicked_ips func
        df_ref.fillna('', inplace=True)
        return df_ref

    def dom_search(self): 
        print(len(self.txt_dom.get("1.0", "end-1c").splitlines()))
        doms = list(set([i.strip() for i in self.txt_dom.get("1.0", "end-1c").splitlines()]))
        print(len(doms))
        doms = [url.replace('www.', '') for url in doms]
        doms = [sub(r"[\[\]]+", "", url) for url in doms]
        doms = [str(urlparse(url).hostname).lower() if urlparse(url).hostname !=None else url.lower() for url in doms]
        doms = [i.strip() for i in doms] # input validations
        #doms = sorted(list(set(doms)), key=lambda x: str(x.replace('-', '')))
        doms.sort(key=lambda x: x.replace('-', ''))
        ...
        try:
            ...
            print(len(mitigations))
            self.df_mit['Domain'] = [i.strip() for i in mitigations]
            self.df_mit.fillna('', inplace=True)
            df_ref = self.df5.loc[self.df5["Domain"].isin(doms)]
            df_ref = df_ref.drop_duplicates('Domain')
            ...
            resp=f"Mitigation Ronin found {len(self.df_mit['Domain'].tolist())} unmitigated domains! Checking the whitelist now"
